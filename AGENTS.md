...
- `apps/hjemmelsok` (`8089`): looks up which hjemmel a politiattest can be required under, from a few words about the purpose. Two steps: a word-overlap search picks candidates out of politiets 164 published formål, and the model chooses among them **by id** - it never writes a hjemmel, and an id it invents is discarded before the answer is sent. With no model reachable the word search's own ordering is the answer and `kilde` says so, so the service degrades instead of failing. Two things set it apart from every other service here. Its rows are politiets own, so it answers `syntetisk: false` in a sandbox where everything else is synthetic. And **it is the one service that calls a language model outside `ai-gateway`** - straight to Telenor AI Factory over the same OpenAI-compatible endpoint the gateway's `telenor-ai-factory` provider uses, sharing its env vars so the key is set once. The gateway's endpoints are one per task and ranking formål candidates is not one of them; the price is that the ranking is absent from `state/ai-trace.jsonl` and `/admin` does not switch its model. `apps/hjemmelsok/README.md` states that trade-off first, and «Videre herfra» there is the way back into the gateway. One caller: `politiattest-flyt` looks a hjemmel up in steg 0 on the kommune page and puts the chosen row into `rettslig_grunnlag` on the formålsbekreftelse - `rolle` and `ordning` stay derived from the case, because those are what the akseptgate on the politiet page compares against the person.
...
- All fourteen Node services (`sandbox-backend`, `demo-gui`, `ai-gateway`, `tools-api`,
  `process-agent`, `fiks-simulator`, `process-builder`, `matrikkel-mock`, `digdir-mock`,
  `pasientjournal-mock`, `politiattest-mock`, `hjemmelsok`, `lommebok`, `politiattest-flyt`) are volume-mounted. The lommebok and politiattest-flyt run their Vite development server via pnpm; the other services run via `scripts/dev.sh`, which selects the right watcher automatically:
...
- All fourteen services have a `healthcheck` in `docker-compose.yml`, and `tools-api`
...
- Contracts: `openapi/README.md`, `openapi/sandbox-backend.yaml`, `openapi/process-agent.yaml`, `openapi/tools-api.yaml`, `openapi/matrikkel-mock.yaml`, `openapi/pasientjournal-mock.yaml`,
  `openapi/politiattest-mock.yaml`, `openapi/hjemmelsok.yaml`, `openapi/ai-gateway.yaml`.
