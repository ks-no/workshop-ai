...
test('admin endpoint is off by default even for a localhost caller', async () => {
  ...
  const postResponse = await POST(postRequest({ role: 'coordinator', provider: 'cloudflare', model: '@cf/qwen/qwen3.8-27b' }));
...
test('GET returns the leverandørkatalog and role status without leaking configured secrets', async () => {
  ...
  assert.deepEqual(body.catalogue.map((entry: { provider: string }) => entry.provider), ['cloudflare', 'telenor-ai-factory']);
  assert.equal(body.model.roles.coordinator.keyConfigured, true);
  assert.equal(body.model.roles.specialist.keyConfigured, true);
...
test('POST switches the active provider and model for a role without a restart, reflected on the next GET', async () => {
  const postResponse = await POST(postRequest({ role: 'specialist', provider: 'telenor-ai-factory', model: 'GLM-5.2' }));
  assert.equal(postResponse.status, 200);
  const posted = await postResponse.json();
  assert.deepEqual(posted.model.roles.specialist, { provider: 'telenor-ai-factory', model: 'GLM-5.2', keyConfigured: true });
  assert.deepEqual(activeSelection('specialist'), { provider: 'telenor-ai-factory', model: 'GLM-5.2' });
  const getResponse = await GET(getRequest());
  assert.deepEqual((await getResponse.json()).model.roles.specialist.model, 'GLM-5.2');
});

test('POST rejects a cross-site request even when the admin function is enabled', async () => {
  const response = await POST(postRequest({ role: 'coordinator', provider: 'cloudflare', model: '@cf/qwen/qwen3.8-27b' }, localOrigin, { Origin: 'http://evil.example' }));
  assert.equal(response.status, 403);
...
test('POST fails closed on an unknown provider or model with 400 and a Norwegian message, and never activates it', async () => {
  const response = await POST(postRequest({ role: 'coordinator', provider: 'telenor-ai-factory', model: 'not-a-real-model' }));
...
test('POST requires a JSON content type', async () => {
  const response = await POST(postRequest({ role: 'coordinator', provider: 'cloudflare', model: '@cf/qwen/qwen3.8-27b' }, localOrigin, { 'Content-Type': 'text/plain' }));
