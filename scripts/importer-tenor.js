...
//
// Usage: node scripts/importer-tenor.js [--tørrkjør] [--glem-id-er]
...
const alder = (foedselsdato) => alderVed(foedselsdato, REFERANSEDATO);
...
function tilBostedsadresse(dokument, kommunenavn) {
...
function buildFamilie(personer, kuraterte) {
  ...
  const kjent = new Set([...personer.keys(), ...kuraterte.map((k) => k.fnr)]);

  const leggTilForelder = (barn, forelder) => {
...
function foreldrerolle(forelderFnr, alleForeldre, kjoennFor) {
...
function adressenoekkel(kommunenummer, adressenavn, husnummer, husbokstav) {
  return [
...
function medMatrikkelId(adresse, indeks) {
  const noekkel = adressenoekkel(
...
async function readIdLedger() {
  ...
  return {
    personId: new Map(eksisterende.map((p) => [p.syntetiskFodselsnummer, p.personId])),
    husstandId: new Map(eksisterende.map((p) => [p.syntetiskFodselsnummer, p.husstandId]))
  };
}
...
function buildHusstander(personer, husstandsnaboer) {
...
function husstandstype(medlemmer) {
  const voksne = medlemmer.filter((m) => alder(m.foedselsdato) >= MYNDIG);
  const barn = medlemmer.length - voksne.length;
  const gift = voksne.some((m) => m.sivilstand === "GIFT");
...
function buildScenario(type, medlemmer) {
  const barnAldre = medlemmer
    .map((m) => alder(m.foedselsdato))
    .filter((a) => a < MYNDIG)
    .sort((a, b) => b - a);
  ...
  if (medlemmer.some((m) => m.adressebeskyttelse !== "UGRADERT")) {
...
function buildEierforhold(husstander, personerUt, matrikkel) {
  const personPerId = new Map(personerUt.map((p) => [p.personId, p]));
  const perMatrikkelId = new Map();
  const antallEid = new Map();

  const leggTil = (matrikkelId, eiere, eierform) => {
    if (!matrikkelId || perMatrikkelId.has(matrikkelId)) return false;
    if (eiere.some((e) => (antallEid.get(e) || 0) >= EIERTAK)) return false;
    const andel = Math.round((1 / eiere.length) * 1000) / 1000;
    perMatrikkelId.set(matrikkelId, {
      matrikkelId,
      eiere: eiere.map((eier) => ({ eier, eierform, andel }))
    });
  ...
  const bebodd = new Set(
    personerUt
      .map((p) => p.bostedsadresse?.adresseIdentifikatorFraMatrikkelen)
      .filter(Boolean)
  );
  ...
  for (const husstand of [...husstander].sort((a, b) => a.husstandId.localeCompare(b.husstandId))) {
    ...
    leggTil(hjem, voksne.map((v) => v.personId), "SELVEIER");
  ...
  return {
    beskrivelse:
      ...
      "adresser deler samme gnr/bnr. Utledet av scripts/importer-tenor.js.",
    syntetisk: true,
    antall: rader.length,
    eierforhold: rader,
    statistikk: {
      husstanderSomEier: rader.filter((r) => r.eiere.some((e) => e.eierform === "SELVEIER")).length,
...
function buildFreg(person, fnrForPersonId) {
  ...
  return {
    ...
    forelderbarnrelasjon: person.relasjoner.map((rel) => ({
...
function assignRoller(medlemmer, naboer) {
  const barn = new Set(medlemmer.filter((m) => alder(m.foedselsdato) < MYNDIG).map((m) => m.fnr));
...
function buildHusstandRad(husstandId, medlemmer, scenario, kilde) {
  ...
  return {
    ...
    medlemmer: medlemmer.map((m) => ({ personId: m.personId, rolle: m.rolle })),
    ...(kilde ? { kilde } : {}),
    syntetisk: true,
...
async function run() {
  ...
  const kuraterteFnrForPersonId = new Map(
    kuratert.personer.map((p) => [p.personId, p.syntetiskFodselsnummer])
  );
  const tilFnr = (personId, eier) => {
  ...
  const kuraterteMedlemmer = kuratert.personer.map((p) => ({
  ...
  const alle = new Map(kuraterteMedlemmer.map((m) => [m.fnr, m]));

  const { foreldreAv, ektefelleAv } = buildFamilie(
    personer,
    kuratert.personer.map((p) => ({
      fnr: p.syntetiskFodselsnummer,
      barn: (p.barn || []).map((b) => tilFnr(b, p.personId)),
  ...
  for (const husstand of kuratert.husstander) {
    const medlemmer = kuraterteMedlemmer
      .filter((m) => m.husstandId === husstand.husstandId)
  ...
  const kuraterteFnr = new Set(kuraterteMedlemmer.map((m) => m.fnr));
  ...
  for (const gruppe of tenorHusstander) {
    ...
    const medlemmer = gruppe.map((fnr) => fraTenor(fnr, husstandId));
  ...
  for (const fnr of [...personer.keys()].sort()) {
    ...
    const utenfor = fraTenor(fnr, null);
    utenfor.rolle = null;
    alle.set(fnr, utenfor);
  }

  // --- relations, derived once for everyone --------------------------------
  const personIdFor = (fnr) => alle.get(fnr)?.personId ?? null;
  const kjoennFor = (fnr) => alle.get(fnr)?.kjoenn ?? null;
