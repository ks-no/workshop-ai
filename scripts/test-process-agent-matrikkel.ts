...
const fakeSession = {
  ...
  stepIndex: 0,
  savedAnswer: null,
  savedAnswers: {} as Record<string, unknown>
};
...
function json(response: any, statusCode: number, data: any) {
...
async function waitFor(url: string, attempts = 40) {
  ...
  return createServer(async (request, response) => {
    ...
    if (request.method === "POST" && request.url === "/mcp/tools/invoke") {
      ...
      if (name === "suggest_step_tools") {
        ...
        const allText = `${steg.id || ""} ${steg.tittel || ""} ${steg.tekst || ""} ${(steg.felter || []).map((f: any) => `${f.label || ""} ${f.placeholder || ""}`).join(" ")}`.toLowerCase();
...
async function run() {
  ...
  try {
    ...
    assert(
      choose.replies.some((r: any) => r.includes("Eksempler på veier i matrikkelen")),
    ...
    assert(
      offTopicAtGate.replies.some((r: any) => r.includes("lagrer dette som utkast")),
      `Agenten lagret ikke tiltaket som utkast ved gate-spørsmål. Fikk: ${JSON.stringify(offTopicAtGate.replies)}`
    );
    assert(
      offTopicAtGate.replies.some((r: any) => r.toLowerCase().includes("fant ikke gaten")),
    ...
    assert(
      metaQuestion.replies.some((r: any) => r.includes("gjenstår")),
    ...
    assert(
      preciseExists.replies.some((r: any) => r.includes("Storgata 5 finnes i matrikkelen")),
    ...
    assert(
      preciseOwner.replies.some((r: any) => r.includes("person-001")),
    ...
    assert(
      lookupQuestion.replies.some((r: any) => r.includes("finnes i matrikkelen")),
    ...
    assert(
      lookupNaturalLanguage.replies.some((r: any) => r.includes("Mener du den gaten") || r.includes("finnes i matrikkelen") || r.includes("Fjøsangerveien")),
    ...
    assert(
      invalidGate.replies.some((r: any) => r.toLowerCase().includes("fant ikke gaten")),
    ...
    assert(
      validGate.replies.some((r: any) => r.includes("Mener du")),
    ...
    assert(
      confirmGate.replies.some((r: any) => r.includes("Flott, jeg bruker Storgata")),
    ...
    assert(
      gateSwitchPrompt.replies.some((r: any) => r.includes("Vil du bytte gate")),
    ...
    assert(
      gateSwitchYes.replies.some((r: any) => r.includes("bytter vi gate til Nordnesveien")),
    ...
    assert(
      boligerYes.replies.some((r: any) => r.includes("svarte tidligere")),
    ...
    assert(
      compactGateInput.replies.some((r: any) => r.includes("Bønesheien")),
