...
for (const daarlig of ["", "1234567890", "123456789012", "1203899000a", "abcdefghijk", null, undefined]) {
  check(`avviser ${JSON.stringify(daarlig)}`, lesFoedselsnummer(daarlig) === null);
}
...
try {
  ...
  kastetForMange = error.message.includes("Fant ikke syntetisk foedselsnummer");
}
...
try {
  ...
  kastetDato = error.message.includes("Ugyldig foedselsdato");
}
...
const bytt = (fnr, i, siffer) => fnr.slice(0, i) + siffer + fnr.slice(i + 1);
check(
...
const ugyldige = personer.filter((p) => !isSyntetiskFoedselsnummer(p.syntetiskFodselsnummer));
check(
  `alle ${personer.length} personer har syntetisk, gyldig fnr`,
  ugyldige.length === 0,
  ugyldige.slice(0, 5).map((p) => `${p.personId}=${p.syntetiskFodselsnummer}`).join(", ")
);
...
const stemmerIkke = personer
  .filter((p) => kuraterteIder.has(p.personId))
  .filter((p) => !stemmerMedFoedselsdato(p.syntetiskFodselsnummer, p.foedselsdato));
check(
  `alle ${kuraterteIder.size} genererte fnr stemmer med foedselsdato`,
  stemmerIkke.length === 0,
  stemmerIkke.slice(0, 5).map((p) => `${p.personId}=${p.syntetiskFodselsnummer}/${p.foedselsdato}`).join(", ")
);
...
const avvik = personer.filter((p) => !stemmerMedFoedselsdato(p.syntetiskFodselsnummer, p.foedselsdato));
check(
  "hvert dato-avvik er en Tenor-person, ingen genererte",
  avvik.every((p) => p.kilde === "tenor"),
  avvik.filter((p) => p.kilde !== "tenor").map((p) => p.personId).join(", ")
);
console.log(
  `  merk: ${avvik.length} Tenor-personer har et fnr som beskriver en annen dato enn ` +
  `foedselsdato (${avvik.map((p) => p.personId).join(", ")}). Det er lovlig i ` +
