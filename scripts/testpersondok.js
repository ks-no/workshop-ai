...
// lie; scripts/valider-data.js fails if it is out of step with the data.
//
...
export function buildTestpersondok(
  ...
  const inntektFnr = new Set(inntekter.map((r) => r.identifikator));
  const eierPersonIder = new Set(
    eierforhold.eierforhold.flatMap((r) => r.eiere.map((e) => e.eier))
  );
  const medPlass = new Map();
  for (const [tjeneste, rader] of Object.entries(plasser)) {
  ...
  const alderFor = (person) => alderVed(person.foedselsdato, referansedato);
  const navnFor = (person) =>
  ...
  const kanSelv = (person) =>
    person.personstatus === "BOSATT" && alderFor(person) >= 18;

  const linjer = [];
  linjer.push("<!-- GENERERT AV scripts/importer-tenor.js. Ikke rediger for hånd. -->");
  ...
  const bosatt = personer.filter((p) => p.personstatus === "BOSATT");
  const medEid = personer.filter(kanEid);
  const mindreaarige = bosatt.filter((p) => alderFor(p) < 18);
  ...
  linjer.push(`| Under 13 — kan aldri logge inn, ingen eID finnes | ${bosatt.filter((p) => alderFor(p) < 13).length} |`);
  linjer.push(`| 67 år eller mer | ${bosatt.filter((p) => alderFor(p) >= 67).length} |`);
  linjer.push(`| Ikke bosatt (død, utflyttet, inaktiv, midlertidig) | ${antall - bosatt.length} |`);
  linjer.push(`| Med adressebeskyttelse | ${personer.filter((p) => p.skjermet).length} |`);
  linjer.push(`| Med inntektsopplysninger | ${personer.filter((p) => inntektFnr.has(p.syntetiskFodselsnummer)).length} |`);
  linjer.push(`| Med registrert eiendom | ${personer.filter((p) => eierPersonIder.has(p.personId)).length} |`);
  linjer.push("");
  linjer.push(
  ...
  for (const person of personer) {
    ...
    const husstand = person.husstandId
      ? `${person.husstandId} (${husstandPerId.get(person.husstandId)?.type ?? "?"})`
      : "—";
    linjer.push(
