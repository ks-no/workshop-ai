...
async function readJson(relativSti) {
  return JSON.parse(await readFile(path.join(repoRoot, relativSti), "utf8"));
}

const kildePersoner = await readJson("data/personer.json");
const kildeHusstander = await readJson("data/husstander.json");
