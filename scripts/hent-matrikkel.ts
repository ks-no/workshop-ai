...
const slug = (verdi: unknown) =>
  normalize(verdi).length > 0 ? normalize(verdi) : "ukjent";

async function hentGate(kommunenummer: string, adressenavn: string): Promise<any> {
  const adresser = [];
  for (let side = 0; ; side += 1) {
    ...
    const json = (await svar.json()) as any;
    ...
    adresser.push(...treff.filter((a: any) => normalize(a.adressenavn) === normalize(adressenavn)));
...
function tilEiendom(adresse: any) {
...
async function run() {
  ...
  const finnes = new Set(
    matrikkel.gater.map((gate: any) => `${gate.kommunenummer}|${normalize(gate.adressenavn)}`)
  );
...
run().catch((feil) => {
  console.error(`Matrikkeluttrekk feilet: ${feilmelding(feil)}`);
  process.exit(1);
});
