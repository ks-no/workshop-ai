...
const mindreaarige = personer.filter(
  (p) => p.personstatus === "BOSATT" && !kanOpptreSelv(p, satser.gjelderFra)
);
const utenRepresentant = mindreaarige.filter(
  (p) => finnRepresentanter(register, p.personId, satser.gjelderFra).length === 0
);
check(
  `alle ${mindreaarige.length} mindreårige har minst én levende representant`,
  utenRepresentant.length === 0,
  utenRepresentant.slice(0, 5).map((p) => p.personId).join(", ")
);
const utenEid = personer.filter((p) => !kanHaEid(p, satser.gjelderFra));
check(
  "det finnes personer uten eID, så sperren har noe å nekte",
  utenEid.length > 0,
  String(utenEid.length)
);
const kanLoggeInn = personer.filter((p) => kanHaEid(p, satser.gjelderFra));
check(
