 ...
 *
 * Usage:
 *   node scripts/test-prosessoekt-concurrency.js
 */
...
function start(name, relativePath, env) {
 ...
 */
async function call(routePath, token, options = {}) {
...
try {
   ...
   */
  const testUsers = await (await fetch(`${digdirUrl}/idporten/testbrukere`)).json();
  const PERSON_IDS = testUsers.filter((user) => user.kanOpptreSelv).slice(0, COUNT).map((user) => user.personId);
  ...
  const created = await Promise.all(
    PERSON_IDS.map((personId, i) =>
      call("/api/prosessoekter", tokens[i], { method: "POST", body: { personId, prosessId: PROSESS } })
    )
  );
  check("alle ti opprettelser gir 201", created.every((s) => s.status === 201),
    created.map((s) => s.status).join(","));

  const ids = created.map((s) => s.body?.oektsId).filter(Boolean);
  ...
  check("alle ti finnes igjen på disk",
    ids.every((id) => onDisk.some((oekt) => oekt.oektsId === id)),
    `${ids.filter((id) => !onDisk.some((oekt) => oekt.oektsId === id)).length} forsvant`);

  // 2. Ten concurrent advances, one per økt. Every stegIndex must reach 1.
  const advanced = await Promise.all(
    ids.map((id, i) => call(`/api/prosessoekter/${id}/neste`, tokens[i], { method: "POST" }))
  );
  check("alle ti neste-kall gir 200", advanced.every((s) => s.status === 200),
    advanced.map((s) => s.status).join(","));

  const after = JSON.parse(await readFile(oektFile, "utf8"));
  check("fortsatt ti økter på disk", after.length === 10, String(after.length));
  const atSteg1 = after.filter((oekt) => oekt.stegIndex === 1);
  check(
  ...
  const wrongOwner = after.filter((oekt) => {
    const expected = created.find((s) => s.body?.oektsId === oekt.oektsId);
