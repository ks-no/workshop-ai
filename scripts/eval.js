...
async function runJudge(check, { text }) {
  ...
  const { score, begrunnelse } = await response.json();
...
async function runCase(dataset, testCase) {
...
async function runDataset(file) {
      ...
      ${s.cases.map((c) => `
        <details style="margin-bottom:6px" ${c.passed ? "" : "open"}>
          ...
          <ul style="margin:6px 0">
            ${c.results.map((r) => `<li style="color:${r.ok ? "#2a6b2a" : "#a32222"}">
...
async function main() {
  ...
  const health = await fetch(`${aiBaseUrl}/helse`).then((r) => r.json()).catch(() => null);
  if (!health) {
...
main().catch((error) => {
  console.error(`Eval feilet: ${error.message}`);
  process.exit(1);
});
