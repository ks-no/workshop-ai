...
**Portene `3000`–`3003`, `8080`–`8089` og `11434` må være ledige.** Er en av dem
...
Fjorten kjørende tjenester, én valgfri avhengighet i kjøretid, sju komplette demo-case. På plass:
...
fjorten tjenestene svarer på `/helse`. Den tar `--reset`, `--reload`, `-d`, `--down` og
...
docker compose up -d --no-deps sandbox-backend fiks-simulator ai-gateway \
  ...
  politiattest-mock hjemmelsok demo-gui process-builder
```
