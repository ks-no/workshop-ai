...
export function loadFlowCase(id?: string): FlowCase {
  ...
  if (session.status === 'thinking' && !locks.has(session.id)) {
...
export function deleteFlowCase(id?: string) {
  if (id && locks.has(id)) throw new CaseError('Vent til planleggingen er ferdig før du sletter saken.', 409);
  if (id) db().prepare('DELETE FROM flows WHERE id = ?').run(id);
}
export async function withFlowLock<T>(id: string, revision: number, work: (session: FlowCase) => Promise<T>): Promise<T> {
  if (locks.has(id)) throw new CaseError('Assistenten arbeider allerede med saken. Vent til den er ferdig.', 409);
