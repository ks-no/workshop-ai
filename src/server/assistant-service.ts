...
import type { AssistantCase, EvidenceSource, MemoryFact, FollowUp, AgentRun, ServiceResult, StructuredAnswer, ServiceId, Outcome } from '../domain/assistant-types';
...
import { callModel, emailPrompt, emailSchema, markModelSuccess, modelName, planSchema, PLANNER_PROMPT, specialistPrompt, specialistSchema, responseLanguageName, type ModelCall } from './assistant-model';
...
export async function draftEmail(session: AssistantCase, serviceId: ServiceId, infer: ModelCall = callModel, persist: Persist = saveAssistantCase) {
  ...
  const run: AgentRun = { id: randomUUID(), agent: 'Skribent', revision: session.revision, status: 'running', startedAt: new Date().toISOString(), completedAt: null, model: modelName('specialist'), durationMs: null, framework: 'Microsoft Agent Framework 1.17.0 · Python' };
