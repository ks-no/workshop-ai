...
import { actionKinds, factKeys, serviceIds, toolIds, type ModelStatus, type ModelPlan, type SpecialistOutput } from '../domain/assistant-types';
...
/** Mirrors provider_configuration() in backend/agent_runtime.py: an OpenAI-compatible endpoint such as the AI Factory LiteLLM proxy. Plain HTTP only on loopback (for example a local Ollama server). */
