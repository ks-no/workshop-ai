...
| `tilgangsstatus.ts` | Klassifiseringen bak `GET /api/personer/{personId}/tilganger`: hvilken av de seks statusene en sak har. Ren og synkron, av samme grunn som `vilkaar.ts`. |
| `tilgangsoversikt.ts` | I/O-halvdelen av den: finner søknad, samtykke og regelresultat per prosess. Kjører reglene gjennom `runRessurs` framfor med sin egen kopi, så samtykkeporten og revisjonsloggen er nøyaktig de samme som i en ekte økt. |
| `samtykke.ts` | Samtykke gitt og trukket utenfor en prosessøkt, for flater som viser hva innbyggeren kan søke på. Formålet og datakildene tas fra prosessens `CONSENT_REQUEST`-steg, og skrivingen går til `fiks-simulator` som i flyten - raden skal ikke røpe hvilken flate den ble til i. |
