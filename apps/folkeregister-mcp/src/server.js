...
function buildSearchIndex(person) {
...
async function loadRegister() {
  ...
  const byFnr = new Map(persons.map((p) => [p.foedselsEllerDNummer, p]));
...
function searchPersons(args, register) {
...
function getPerson(args, register) {
  const fnr = String(args.foedselsEllerDNummer || args.fnr || "").trim();
...
async function main() {
    ...
    try {
      ...
      if (message.method === "tools/call") {
        ...
        if (name === "folkeregister_search_persons") {
          process.stdout.write(encodeMessage(createSuccessResponse(message.id, toolTextResult(searchPersons(args, register)))));
          return;
        }

        if (name === "folkeregister_get_person") {
          process.stdout.write(encodeMessage(createSuccessResponse(message.id, toolTextResult(getPerson(args, register)))));
          return;
        }
  ...
  const reader = createMessageReader((message) => { void handleMessage(message); });
  process.stdin.on("data", reader);
  process.stdin.on("error", (error) => { process.stderr.write(`stdin error: ${error.message}\n`); });
  process.stderr.write(`folkeregister-mcp ready with ${register.persons.length} persons from ${dataFile}\n`);
}

main().catch((error) => {
  process.stderr.write(`Failed to start folkeregister-mcp: ${error.message}\n`);
  process.exit(1);
});
